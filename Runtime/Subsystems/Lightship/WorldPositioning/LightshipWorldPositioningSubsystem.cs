...
namespace Niantic.Experimental.Lightship.AR.Subsystems.WorldPositioning
{
    ...
    public sealed class LightshipWorldPositioningSubsystem : XRWorldPositioningSubsystem
    {
        internal class LightshipProvider : Provider
        {
            ...
            public override WorldPositioningStatus TryGetXRToWorld(ref Matrix4x4 arToWorld, ref double originLatitude, ref double originLongitude, ref double originAltitude)
            {
                if(!running)
                {
                ...
                return  (WorldPositioningStatus) _api.TryGetXRToWorld(_nativeProviderHandle, out arToWorld, out originLatitude, out originLongitude, out originAltitude);
            }
        }
        ...
        static void RegisterDescriptor()
        {
            ...
            SubsystemDescriptorStore.RegisterDescriptor(XRWorldPositioningSubsystemDescriptor.Create(cinfo));

            foreach(XRLoader existingLoader in XRGeneralSettings.Instance.AssignedSettings.activeLoaders)
                if (existingLoader is ILightshipLoader)
                {
                    (existingLoader as ILightshipLoader).AddExternalLoader(new LightshipWorldPositioningLoader());
                }
        }
