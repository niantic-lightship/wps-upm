...
namespace Niantic.Experimental.Lightship.AR.XRSubsystems
{
    /// <summary>
    ...
    public struct XRWorldPositioningSubsystemCinfo : IEquatable<XRWorldPositioningSubsystemCinfo>
    {
        ...
        public override bool Equals(System.Object obj) => ((obj is XRWorldPositioningSubsystemCinfo) && Equals((XRWorldPositioningSubsystemCinfo)obj));
        ...
        public static bool operator ==(XRWorldPositioningSubsystemCinfo lhs, XRWorldPositioningSubsystemCinfo rhs) => lhs.Equals(rhs);
    ...
    public class XRWorldPositioningSubsystemDescriptor :
        SubsystemDescriptorWithProvider<XRWorldPositioningSubsystem, XRWorldPositioningSubsystem.Provider>
    {
        ...
        internal static XRWorldPositioningSubsystemDescriptor Create(XRWorldPositioningSubsystemCinfo wpsSubsystemCinfo)
        {
            if (string.IsNullOrEmpty(wpsSubsystemCinfo.id))
            {
                throw new ArgumentException("Cannot create World Space subsystem descriptor because id is invalid",
                                            nameof(wpsSubsystemCinfo));
            }

            if (wpsSubsystemCinfo.providerType == null
                || !wpsSubsystemCinfo.providerType.IsSubclassOf(typeof(XRWorldPositioningSubsystem.Provider)))
            {
                throw new ArgumentException("Cannot create World Space subsystem descriptor because providerType is invalid",
                                            nameof(wpsSubsystemCinfo));
            }

            if (wpsSubsystemCinfo.subsystemTypeOverride == null
                || !wpsSubsystemCinfo.subsystemTypeOverride.IsSubclassOf(typeof(XRWorldPositioningSubsystem)))
            {
                throw new ArgumentException("Cannot create World Space subsystem descriptor because subsystemTypeOverride is invalid",
                                            nameof(wpsSubsystemCinfo));
            }
